<html>
<head>
<title>b&ouml;hni's Profil</title>
</head>
<body>
	<h2>b&ouml;hni's Profil</h2>
	<table border="0" height="350" width="500">
		<!-- 1.Tabellenreihe mit dem User-Avatar & Vorname-->
		<tr height="25">
			<td rowspan="5"><img src="\user_pictures\user1234.jpg" width="100" height="100"></td>
			<td align="right">Vorname:</td>
			<td>Benjamin</td>
		</tr>
		<!-- 2.Tabellenreihe fuer die Nachname-->
		<tr height="25">
			<td align="right">Nachname:</td>
			<td>B&ouml;hnstett</td>
		</tr>
		<!-- 3.Tabellenreihe f�r die Alter-->
		<tr height="25">
			<td align="right">Alter:</td>
			<td>18 Jahre</td>
		</tr>
		<!-- 4.Tabellenreihe f�r die Nickname-->
		<tr height="25">
			<td align="right">Nickname:</td>
			<td>b&ouml;hni</td>
		</tr>
        <!-- 5.Tabellenreihe f�r die E-Mail-->
		<tr height="25">
			<td align="right">E-Mail:</td>
			<td>b&ouml;hni@web.de</td>
		</tr>
		<!-- 6.Tabellenreihe Unter�bschreift f�r den about me-teil-->
		<tr height="25">
			<td colspan="3"><br><br><h3>Infos &uuml;ber mich:</h3></td>
		</tr>
		<!-- 7.Tabellenreihe die "about me"-Eingabe -->
		<tr>
			<td colspan="3"><textarea style="background-color:white;border:white;"
                                        name="profiltext" cols="60" rows="10"
                                        readonly>Test Test Test </textarea></td>
		</tr>
	</table>
</body>
</html>